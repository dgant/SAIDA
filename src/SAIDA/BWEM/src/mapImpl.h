...
namespace BWEM {
	...
	namespace detail {
		...
		public:
			...
			vector<ChokePoint *>		GetAllChokePoints() const override {
				return GetGraph().ChokePoints();
			}
			...
			const CPPath 				&GetPath(const BWAPI::Position &a, const BWAPI::Position &b, int *pLength = nullptr, bool fastSearch = true) const override {
				return m_Graph.GetPath(a, b, fastSearch, pLength);
			}
