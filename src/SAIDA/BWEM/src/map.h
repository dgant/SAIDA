...
namespace BWEM
{
	...
	public:
		...
		const BWAPI::TilePosition 			&Size() const								{
			return m_Size;
		}

		// Returns the size of the Map in MiniTiles.
		const BWAPI::WalkPosition 			&WalkSize() const							{
			return m_WalkSize;
		}
		...
		virtual vector<ChokePoint *>		GetAllChokePoints() const = 0;
		// Returns the number of ChokePoints.
		virtual int							ChokePointCount() const = 0;

		// Returns a Tile, given its position.
		const Tile 						&GetTile(const BWAPI::TilePosition &p, utils::check_t checkMode = utils::check_t::check) const	{
		...
		const MiniTile 					&GetMiniTile(const BWAPI::WalkPosition &p, utils::check_t checkMode = utils::check_t::check) const	{
			bwem_assert((checkMode == utils::check_t::no_check) || Valid(p));
			utils::unused(checkMode);
		...
		const std::vector<MiniTile> 		&MiniTiles() const								{
			return m_MiniTiles;
		}

		// Returns whether the position p is valid.
		bool								Valid(const BWAPI::TilePosition &p) const		{
			return (0 <= p.x) && (p.x < Size().x) && (0 <= p.y) && (p.y < Size().y);
		}
		bool								Valid(const BWAPI::WalkPosition &p) const		{
			return (0 <= p.x) && (p.x < WalkSize().x) && (0 <= p.y) && (p.y < WalkSize().y);
		}
		bool								Valid(const BWAPI::Position &p) const			{
			return Valid(BWAPI::WalkPosition(p));
		}
		...
		virtual const CPPath 				&GetPath(const BWAPI::Position &a, const BWAPI::Position &b, int *pLength = nullptr, bool fastSearch = true) const = 0;
	...
	protected:
		...
		Tile 								&GetTile_(const BWAPI::TilePosition &p, utils::check_t checkMode = utils::check_t::check)		{
			return const_cast<Tile &>(static_cast<const Map &>(*this).GetTile(p, checkMode));
		}
		MiniTile 							&GetMiniTile_(const BWAPI::WalkPosition &p, utils::check_t checkMode = utils::check_t::check)	{
