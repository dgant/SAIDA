...
namespace BWEM {
	...
	public:
		...
		const BWAPI::TilePosition 		&TopLeft() const					{
			return m_topLeft ;
		}
		const BWAPI::TilePosition 		&BottomRight() const				{
		...
		const std::vector<const ChokePoint *> 	&ChokePoints() const		{
		...
		const std::vector<const Area *> &AccessibleNeighbours() const	{
		...
		// See also GroupId()
		bool							AccessibleFrom(const Area *pArea) const	{
			return GroupId() == pArea->GroupId();
		}
		...
		void							setIsland(bool island) const {
			m_island = island;
		}
		void							calcBoundaryVertices() const;
		std::vector<BWAPI::Position>	getBoundaryVertices() const
		{
