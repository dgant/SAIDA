...
namespace BWEM {
	...
	public:
		...
		bool				SeaOrLake() const			{
			return m_altitude == 1;
		}
		void				SetSea()					{
			bwem_assert(!Walkable() && SeaOrLake());
			m_altitude = 0;
		}
		void				SetLake()					{
			bwem_assert(!Walkable() && Sea());
			m_altitude = -1;
		}
		bool				AltitudeMissing() const		{
			return m_altitude == -1;
		}
		...
		bool				AreaIdMissing() const		{
			return m_areaId == -1;
		}
		void				SetAreaId(Area::id id)		{
			bwem_assert(AreaIdMissing() && (id >= 1));
			m_areaId = id;
		}
		void				ReplaceAreaId(Area::id id)	{
			bwem_assert((m_areaId > 0) && ((id >= 1) || (id <= -2)) && (id != m_areaId));
			m_areaId = id;
		}
		void				SetBlocked()				{
			bwem_assert(AreaIdMissing());
			m_areaId = blockingCP;
		}
		bool				Blocked() const				{
			return m_areaId == blockingCP;
		}
		void				ReplaceBlockedAreaId(Area::id id)	{
			bwem_assert((m_areaId == blockingCP) && (id >= 1));
			m_areaId = id;
		}
	...
	public:
		...
		void				AddNeutral(Neutral *pNeutral)	{
			bwem_assert(!m_pNeutral && pNeutral);
			m_pNeutral = pNeutral;
		}
		void				SetAreaId(Area::id id)			{
			bwem_assert((m_areaId > id) || !m_areaId && id);
			m_areaId = id;
		}
		void				ResetAreaId()					{
			m_areaId = 0;
		}
		void				SetMinAltitude(altitude_t a)	{
			bwem_assert(a >= 0);
			m_minAltitude = a;
		}
		void				RemoveNeutral(Neutral *pNeutral) {
			bwem_assert(pNeutral && (m_pNeutral == pNeutral));
			utils::unused(pNeutral);
			m_pNeutral = nullptr;
		}
		int					InternalData() const			{
			return m_internalData;
		}
		void				SetInternalData(int data) const	{
			m_internalData = data;
		}
