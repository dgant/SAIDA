...
namespace BWEM {
	...
	namespace utils
	{
		...
		public:
			...
			bool			TopBorder() const		{
				return m_topBorder;
			}
			bool			BottomBorder() const	{
				return m_bottomBorder;
			}
			bool			LeftBorder() const		{
				return m_leftBorder;
			}
			bool			RightBorder() const		{
												...
												for (int y = minY ; y <= maxY ; ++y)
													for (int x = minX ; x <= maxX ; ++x)
													{
														if (wallSize == 3)
														{
															...
															for (const auto &BuildableBorderTiles : {
															BuildableBorderTiles1, BuildableBorderTiles2
														})
