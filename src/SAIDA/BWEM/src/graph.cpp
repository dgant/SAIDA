	...
	namespace detail {
				...
				if (!pNeutral->NextStacked())		// in the case where several neutrals are stacked, we only consider the top
				{
					...
					for (const Area *pA : BlockedAreas)
						for (const Area *pB : BlockedAreas)
						{
							...
							[](const MiniTile & miniTile, WalkPosition) {
								return miniTile.Walkable();
							},	// findCond
							[](const MiniTile &,          WalkPosition) {
								return true;
							});					// visitCond
		...
		vector<int> Graph::ComputeDistances(const ChokePoint *start, const vector<const ChokePoint *> &Targets) const
		{
			...
			while (!ToVisit.empty())
			{
				...
				for (const Area *pArea : {
				current->GetAreas().first, current->GetAreas().second
				})
						...
						if (!nextTile.Marked())
						{
							if (nextTile.InternalData())	// next already in ToVisit
							{
								...
								{	// To update next's distance, we need to remove-insert it from ToVisit:
									...
									(const pair<int, const ChokePoint *> &e) {
		...
		const CPPath &Graph::GetPath(const Position &a, const Position &b, bool fastSearch, int *pLength) const
		{
			...
			const Area *pAreaA = nullptr;
			const Area *pAreaB = nullptr;

			if (fastSearch) {
				pAreaA = GetArea(WalkPosition(a));
				...
				pAreaA = GetNearestArea(WalkPosition(a));
				pAreaB = GetNearestArea(WalkPosition(b));
			}
		...
		void Graph::CreateBases()
		{
			...
			for (Area &area : m_Areas)
			{
				...
				for (Base &base : area.Bases()) {
					TilePosition min = TilePosition(1000, 1000);
					TilePosition max = TilePosition(0, 0);

					Position avg = Position(0, 0);

					for (const Mineral *mineral : base.Minerals()) {
						avg += mineral->Pos();

						if (min.x > mineral->TopLeft().x) {
							base.setLeftMineral(mineral->Unit());
							min.x = mineral->TopLeft().x;
						}

						if (min.y > mineral->TopLeft().y) {
							base.setTopMineral(mineral->Unit());
							min.y = mineral->TopLeft().y;
						}

						if (max.x < mineral->BottomRight().x) {
							base.setRightMineral(mineral->Unit());
							max.x = mineral->BottomRight().x;
						}

						if (max.y < mineral->BottomRight().y) {
							base.setBottomMineral(mineral->Unit());
							max.y = mineral->BottomRight().y;
						}
					}

					base.setMineralBaseCenter(((avg / base.Minerals().size()) + base.Center()) / 2);
					base.setBaseLocationInfo();
					base.setIsMineralsPlacedVertical();
					base.setIsExistBackYard((!base.isMineralsLeftOfBase() && area.BottomRight().x + 1 - max.x >= 3) || (base.isMineralsLeftOfBase() && min.x - area.TopLeft().x >= 3));
				}
			}
		}

		void Graph::ComputeIslandArea() {
			for (Area &area : m_Areas) {
				if (area.ChokePoints().empty())
				{
					area.setIsland(true);
				}
				else {
					int cpCnt = area.ChokePoints().size();

					for (auto cp : area.ChokePoints()) {
						if (cp->Pos(ChokePoint::end1).getApproxDistance(cp->Pos(ChokePoint::end2)) < 4) {
							cpCnt--;
						}
					}
