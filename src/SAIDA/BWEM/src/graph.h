...
namespace BWEM {
	...
	namespace detail {
		...
		public:
			...
			const Area 						*GetArea(Area::id id) const		{
			...
			Area 								*GetArea(BWAPI::WalkPosition w)	{
				return const_cast<Area *>(static_cast<const Graph &>(*this).GetArea(w));
			}

			const Area 						*GetArea(BWAPI::TilePosition t) const;
			Area 								*GetArea(BWAPI::TilePosition t)	{
				return const_cast<Area *>(static_cast<const Graph &>(*this).GetArea(t));
			}

			template<class TPosition>const Area *GetNearestArea(TPosition p) const;
			template<class TPosition> Area 	*GetNearestArea(TPosition p)		{
				return const_cast<Area *>(static_cast<const Graph &>(*this).GetNearestArea(p));
			}


			// Returns the list of all the ChokePoints in the Map.
			const vector<ChokePoint *> 		&ChokePoints() const				{
			...
			const vector<ChokePoint> 			&GetChokePoints(const Area *a, const Area *b) const	{
				return GetChokePoints(a->Id(), b->Id());
			}

			// Returns the ground distance in pixels between cpA->Center() and cpB>Center()
			int									Distance(const ChokePoint *cpA, const ChokePoint *cpB) const {
				return m_ChokePointDistanceMatrix[cpA->Index()][cpB->Index()];
			}

			// Returns a list of ChokePoints, which is intended to be the shortest walking path from cpA to cpB.
			const CPPath 						&GetPath(const ChokePoint *cpA, const ChokePoint *cpB) const {
				return m_PathsBetweenChokePoints[cpA->Index()][cpB->Index()];
			}

			const CPPath 						&GetPath(const BWAPI::Position &a, const BWAPI::Position &b, bool fastSearch, int *pLength) const;
			...
			vector<ChokePoint> 				&GetChokePoints(Area::id a, Area::id b)			{
				return const_cast<vector<ChokePoint> &>(static_cast<const Graph &>(*this).GetChokePoints(a, b));
			}
			vector<ChokePoint> 				&GetChokePoints(const Area *a, const Area *b) 	{
				return GetChokePoints(a->Id(), b->Id());
			}
