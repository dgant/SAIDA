...
namespace BWEM {
	namespace detail {
				...
				for (int x = 0 ; x < WalkSize().x ; ++x)
				{
					...
					if (Origin.SeaOrLake())
					{
						...
						while (!ToSearch.empty())
						{
							...
							for (WalkPosition delta : {
										WalkPosition(0, -1), WalkPosition(-1, 0), WalkPosition(+1, 0), WalkPosition(0, +1)
									})
							{
			...
			for (const auto &delta_altitude : DeltasByAscendingAltitude)
			{
				...
				for (int i = 0 ; i < (int)ActiveSeaSideList.size() ; ++i)
				{
					...
					if (altitude - Current.lastAltitudeGenerated >= 2 * altitude_scale)		// optimization : once a seaside miniTile verifies this condition,
						...
						for (auto delta : {
									WalkPosition(d.x, d.y), WalkPosition(-d.x, d.y), WalkPosition(d.x, -d.y), WalkPosition(-d.x, -d.y),
			...
			for (Neutral *pCandidate : Candidates)
				if (!pCandidate->NextStacked())		// in the case where several neutrals are stacked, we only consider the top one
				{
					...
					while (!Border.empty())
					{
						...
						while (!ToVisit.empty())
						{
							WalkPosition current = ToVisit.back();
							ToVisit.pop_back();
						...
						really_remove_if(Border, [&Visited](WalkPosition w)	{
							return contains(Visited, w);
						});
					}
		...
		{
		public:
			TempAreaInfo() : m_valid(false), m_id(0), m_top(0, 0), m_highestAltitude(0) {
				bwem_assert(!Valid());
			}
			...
			altitude_t			HighestAltitude() const			{
				bwem_assert(Valid());
				return m_highestAltitude;
			}

			void				Add(MiniTile *pMiniTile)		{
				bwem_assert(Valid());
				++m_size;
			...
			void				Merge(TempAreaInfo &Absorbed)	{
				...
				if (m_highestAltitude < Absorbed.m_highestAltitude) {
					m_highestAltitude = Absorbed.m_highestAltitude;
					m_top = Absorbed.m_top;
				}
			}
		...
		vector<pair<WalkPosition, MiniTile *>> MapImpl::SortMiniTiles()
		{
			...
			[](const pair<WalkPosition, MiniTile *> &a, const pair<WalkPosition, MiniTile *> &b) {
				return a.second->Altitude() > b.second->Altitude();
			});
		...
		vector<TempAreaInfo> MapImpl::ComputeTempAreas(const vector<pair<WalkPosition, MiniTile *>> &MiniTilesByDescendingAltitude)
		{
			...
			for (const auto &Current : MiniTilesByDescendingAltitude)
			{
				...
				else									// two neighboring areas : adds cur to one of them  &  possible merging
				{
					...
					if ((TempAreaList[smaller].Size() < 90) ||
		...
		{
			auto iMineral = find_if(m_Minerals.begin(), m_Minerals.end(), [u](const unique_ptr<Mineral> &m) {
				return m->Unit() == u;
			});
		...
		{
			auto iGeyser = find_if(m_Geysers.begin(), m_Geysers.end(), [u](const unique_ptr<Geyser> &g) {
				return g->Unit() == u;
			});
		...
		{
			auto iStaticBuilding = find_if(m_StaticBuildings.begin(), m_StaticBuildings.end(), [u](const unique_ptr<StaticBuilding> &g) {
				return g->Unit() == u;
			});
