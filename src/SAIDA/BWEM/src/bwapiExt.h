...
namespace BWEM {
	...
	namespace BWAPI_ext {
		...
		inline std::ostream & operator<<(std::ostream &out, BWAPI::Point<T, Scale> A)		{
			out << "(" << A.x << ", " << A.y << ")";
			return out;
		}
		...
		template<typename T, int Scale = 1>
		inline BWAPI::Position center(BWAPI::Point<T, Scale> A)	{
			return BWAPI::Position(A) + BWAPI::Position(Scale / 2, Scale / 2);
		}


		template<typename T, int Scale = 1>
		inline BWAPI::Point<T, Scale> operator+(BWAPI::Point<T, Scale> A, int b)	{
			return A + BWAPI::Point<T, Scale>(b, b);
		}

		template<typename T, int Scale = 1>
		inline BWAPI::Point<T, Scale> operator+(int a, BWAPI::Point<T, Scale> B)	{
			return B + a;
		}
		...
		template<typename T, int Scale = 1>
		inline int queenWiseDist(BWAPI::Point<T, Scale> A, BWAPI::Point<T, Scale> B) {
			A -= B;
			return utils::queenWiseNorm(A.x, A.y);
		}

		template<typename T, int Scale = 1>
		inline int squaredDist(BWAPI::Point<T, Scale> A, BWAPI::Point<T, Scale> B)	{
			A -= B;
			return squaredNorm(A.x, A.y);
		}

		template<typename T, int Scale = 1>
		inline double dist(BWAPI::Point<T, Scale> A, BWAPI::Point<T, Scale> B)		{
			A -= B;
			return utils::norm(A.x, A.y);
		}

		template<typename T, int Scale = 1>
		inline int roundedDist(BWAPI::Point<T, Scale> A, BWAPI::Point<T, Scale> B)	{
		...
		inline int distToRectangle(const BWAPI::Position &a, const BWAPI::TilePosition &TopLeft, const BWAPI::TilePosition &Size)
		{
			...
			if (a.x >= topLeft.x)
				...
				else if (a.y > bottomRight.y)	return roundedDist(a, bottomRight);									// SE
				else if (a.y < topLeft.y)	return roundedDist(a, BWAPI::Position(bottomRight.x, topLeft.y));	// NE
				else						return a.x - bottomRight.x;											// E
			else if (a.y > bottomRight.y)		return roundedDist(a, BWAPI::Position(topLeft.x, bottomRight.y));	// SW
